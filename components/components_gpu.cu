                               ...
                               Context ctx, Runtime *runtime)
{
  ...
  for (V_ID n = 0; n < graph->nv; n++) {
    while ((cur < colRight - colLeft + 1) && (edges[cur].src <= n))
      cur ++;
    nodes[n].index = cur;
  }
  ...
  char* bitmap = (char*)header + sizeof(FrontierHeader);
