...
__global__
void sssp_pull_kernel(V_ID rowLeft,
                      V_ID rowRight,
                      E_ID colLeft,
       ...
       blkRowStart += blockDim.x * gridDim.x)
  {
    ...
    while (totalNumEdges > 0) {
      if (threadIdx.x < totalNumEdges) {
        ...
        atomicMin(new_pr_fb + es.dst - rowLeft, srcLabel + 1);
      }
...
__global__
void sssp_push_kernel(V_ID inRowLeft,
                    V_ID inRowRight,
                    V_ID myRowLeft,
                    E_ID colLeft,
                    const NodeStruct* row_ptrs,
                        ...
                        Context ctx, Runtime *runtime)
{
  ...
  if (oldFqSize > graph->nv / 16) {
    ...
    sssp_pull_kernel<<<GET_BLOCKS(rowRight - rowLeft + 1), CUDA_NUM_THREADS>>>(
    ...
    for (int i = 0; i < graph->numParts; i ++) {
      ...
      if (old_header->type == FrontierHeader::DENSE_BITMAP) {
        ...
        sssp_push_kernel<<<numBlocks, CUDA_NUM_THREADS, 0, piece->streams[i]>>>(
