...
LegionRuntime::Logger::Category log_cf("colfilter");
                    ...
                    Context ctx, HighLevelRuntime *runtime)
{
  ...
  // parse input arguments
  {
    ...
    log_cf.print("Collaborative Filtering settings: numPartitions(%d) numIter(%d)" 
  ...
  // Computation Phase
  int iteration = 0;
  log_cf.print("Start Collaborative Filtering computation...");
  ...
  log_cf.print("Finish Collaborative Filtering computation...");
...
int main(int argc, char **argv)
{
  ...
  {
    TaskVariantRegistrar registrar(APP_TASK_ID, "colfilter_task");
    registrar.add_constraint(ProcessorConstraint(Processor::TOC_PROC));
    registrar.set_leaf();
    Runtime::preregister_task_variant<app_task_impl>(
        registrar, "colfilter_task");
  }
